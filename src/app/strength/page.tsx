...
import { Check, Building2, Sparkles, X, CheckCircle2 } from "lucide-react";
...
export const metadata: Metadata = {
  title: "強み｜WEBKURA",
  description: "制作会社に頼むより、速く・自由に・安く。現役エンジニアが直接開発するWEBKURAならではの4つの強みと、制作会社との比較を紹介します。",
};

const STRENGTHS: [string, string, string, string, string?][] = [
  ["01", "最短1〜2週間の圧倒的スピード", "一般的な制作会社は営業・ディレクター・デザイナー・エンジニアと確認工程が多く1〜3ヶ月かかりがち。WEBKURAは少人数体制で意思決定が速く、Basicプランなら最短1週間で公開できます。要件が固まっている案件では着手から数日で初稿をお見せすることも可能です。", "最短納期 1週間〜"],
  ...
  return (
    <main style={{ background: "#f4f6f8", color: "#0e1b2c", overflowX: "hidden" }}>
      ...
      <PageHero
        ...
        lead="WEBKURAは現役エンジニアが直接手がけるホームページ制作サービスです。制作会社を通さず直接依頼いただくことで、短納期・低コスト・高い自由度を同時に実現します。"
      />
      ...
      <section style={{ padding: "clamp(70px,11vh,130px) clamp(20px,4vw,56px)", background: "#fff" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {STRENGTHS.map(([num, title, body, , note], i) => (
              <div key={num} style={{ display: "grid", gridTemplateColumns: "140px 1fr 1.2fr", gap: "clamp(20px,3vw,48px)", padding: "44px 0", borderTop: i === 0 ? "1.5px solid #0e1b2c" : "1px solid rgba(14,27,44,0.10)", borderBottom: i === STRENGTHS.length - 1 ? "1.5px solid #0e1b2c" : undefined, alignItems: "start" }}>
      ...
      <section style={{ padding: "clamp(70px,11vh,130px) clamp(20px,4vw,56px)", background: "#f4f6f8" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0,1fr))", gap: 1, background: "rgba(14,27,44,0.10)", border: "1px solid rgba(14,27,44,0.10)", borderRadius: 16, overflow: "hidden" }}>
            {AUDIENCE.map((item) => (
              <div key={item} style={{ background: "#fff", padding: "24px 28px", display: "flex", gap: 14, alignItems: "flex-start", fontSize: "0.92rem", lineHeight: 1.9, fontWeight: 500 }}>
                <span style={{ flexShrink: 0, width: 22, height: 22, marginTop: 2, borderRadius: "50%", background: "#2f6bff", display: "flex", alignItems: "center", justifyContent: "center" }}><Check size={13} color="#fff" strokeWidth={3} /></span>{item}
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="compare" style={{ padding: "clamp(70px,11vh,130px) clamp(20px,4vw,56px)", background: "#fff", scrollMarginTop: 90 }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 1fr", border: "1px solid rgba(14,27,44,0.10)", borderRadius: 20, overflow: "hidden", minWidth: 640, boxShadow: "0 16px 40px rgba(14,27,44,0.08)" }}>
              <div style={{ padding: "20px 20px", background: "#fff" }} />
              <div style={{ padding: "20px 20px", background: "#f4f6f8", fontWeight: 900, fontSize: "1rem", borderLeft: "1px solid rgba(14,27,44,0.10)", display: "flex", alignItems: "center", gap: 10 }}><Building2 size={18} color="#5a6b80" />一般的な制作会社</div>
              <div style={{ padding: "20px 20px", background: "linear-gradient(135deg,#0e1b2c 0%,#1c2f57 100%)", color: "#fff", fontWeight: 900, fontSize: "1rem", borderLeft: "1px solid rgba(14,27,44,0.10)", display: "flex", alignItems: "center", gap: 10 }}><Sparkles size={18} color="#ff9d7a" />WEBKURA</div>
              {COMPARE.map(([label, agency, us], i) => (
                <React.Fragment key={label}>
                  <div style={{ padding: "20px 20px", fontWeight: 700, fontSize: "0.9rem", borderTop: "1px solid rgba(14,27,44,0.10)", borderBottom: i === COMPARE.length - 1 ? "1px solid rgba(14,27,44,0.10)" : undefined, display: "flex", alignItems: "center" }}>{label}</div>
                  <div style={{ padding: "20px 20px", fontSize: "0.86rem", color: "#7a8798", borderTop: "1px solid rgba(14,27,44,0.10)", borderLeft: "1px solid rgba(14,27,44,0.10)", borderBottom: i === COMPARE.length - 1 ? "1px solid rgba(14,27,44,0.10)" : undefined, display: "flex", alignItems: "flex-start", gap: 8 }}><X size={16} color="#c3ccd6" style={{ flexShrink: 0, marginTop: 2 }} />{agency}</div>
                  <div style={{ padding: "20px 20px", background: "rgba(47,107,255,0.05)", fontSize: "0.9rem", fontWeight: 700, color: "#0e1b2c", borderTop: "1px solid rgba(14,27,44,0.10)", borderLeft: "1px solid rgba(14,27,44,0.10)", borderBottom: i === COMPARE.length - 1 ? "1px solid rgba(14,27,44,0.10)" : undefined, display: "flex", alignItems: "flex-start", gap: 8 }}><CheckCircle2 size={16} color="#2f6bff" style={{ flexShrink: 0, marginTop: 2 }} />{us}</div>
                </React.Fragment>
              ))}
            </div>
          </div>
          <div style={{ marginTop: 56, padding: "32px 36px", border: "1.5px solid #0e1b2c", borderRadius: 20, boxShadow: "0 6px 20px rgba(14,27,44,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 20 }}>
            ...
            <Link href="/price" style={{ background: "linear-gradient(135deg,#0e1b2c 0%,#1c2f57 100%)", color: "#fff", fontWeight: 700, fontSize: "0.95rem", borderRadius: 999, padding: "16px 36px", display: "inline-flex", alignItems: "center", gap: 10 }}>料金ページを見る<span>→</span></Link>
          </div>
        </div>
      </section>
